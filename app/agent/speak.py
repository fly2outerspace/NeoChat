...
from app.prompt.speak import SYSTEM_PROMPT, ROLEPLAY_PROMPT
from app.memory import Memory
from app.utils import get_current_time
from app.utils.mapping import CATEGORY_TO_INDICATOR_MAP
from app.storage.scenario_store import ScenarioStore
from app.prompt.speak import HELPER_PROMPT

class SpeakAgent(ChatAgent):
    """Speak agent class that extends ChatAgent with voice/spoken conversation prompts"""

    name: str = "speak"
    description: str = "an agent that communicates through spoken conversation."
    system_prompt: str = SYSTEM_PROMPT
    
    # Message type for streaming events
    message_type: str = ToolName.SPEAK_IN_PERSON
    
    # Speak-specific attributes
            ...
            combined_items.append(
                {
                    ...
                    "end_at": sc.end_at,
                    "text": (getattr(sc, "title", "") or sc.content or ""),
                }
            )
    ...
    def add_assistant_message(self, content: str, created_at: Optional[str] = None) -> None:
        """Add assistant message with SPEAK_IN_PERSON category"""
        ...
        assistant_msg = Message.assistant_message(
            ...
            category=MessageCategory.SPEAK_IN_PERSON,
            visible_for_characters=self.visible_for_characters
        )
